...
for (var i = 0; i < btn_update.length; i++) {
    ...
    btn_update[i].onclick = function () {
        ...
        document.getElementById("vendor_name_update").value = td[0].innerHTML;
        document.getElementById("vendor_key_update").value = td[1].innerHTML;
    }
}
...
for (var i = 0; i < btn_delete.length; i++) {
    ...
    btn_delete[i].onclick = function () {
        ...
        var tr = table.getElementsByTagName("tr")[this.index + 1];
        var td = tr.getElementsByTagName("td");
        document.getElementById("vendor_name_delete").value = td[0].innerHTML;
        document.getElementById("vendor_key_delete").value = td[1].innerHTML;
    }
}
