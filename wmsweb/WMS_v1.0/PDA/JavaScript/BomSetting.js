...
for (var i = 0; i < btn_update.length; i++) {
    ...
    btn_update[i].onclick = function () {
        ...
        //update界面显示原来内容
        document.getElementById("requirement_line_Update").value = td[0].innerHTML;
        document.getElementById("item_name_Update").value = td[1].innerHTML;
        document.getElementById("operation_seq_num_Update").value = td[2].innerHTML;
        document.getElementById("required_qty_Update").value = td[3].innerHTML;
...
for (var i = 0; i < btn_delete.length; i++) {
    ...
    btn_delete[i].onclick = function () {
        ...
        document.getElementById("requirement_line_Delet").value = td.innerHTML;
    }
}
...
$(document).ready(function () {
    $.ajax({
        ...
        dataType: "json",
        data: {
            warehouse: $("#requirement_line_Query").val(),
        },
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                $("#requirement_line_Query").append("<option>" + data[i].Name + "</option>");
            }
        }
    });
});
...
$(document).ready(function () {
    $.ajax({
        ...
        dataType: "json",
        data: {
            warehouse: $("#requirement_line_Insert").val(),
        },
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                $("#requirement_line_Insert").append("<option>" + data[i].Name + "</option>");
            }
        }
    });
});
