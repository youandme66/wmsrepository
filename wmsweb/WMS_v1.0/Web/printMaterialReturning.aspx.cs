...
{
    public partial class printMaterialReturning : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            //判断退料單號是否存在，将根据退料單號进行一下所有操作
            if (Session["select_text_print"] == null)
                PageUtil.showAlert(this, "请输入退料單號，再执行此打印操作");
            else
            {
        ...
        protected void DataView()
        {
            ...
            try
            {
                ...
                dataset_header = invoiceDC.getReturnHeaderBySome(select_text.Value, "", "");
                ...
                return_type.Value = dataset_header.Tables[0].Rows[0]["return_type"].ToString();
                status.Value = dataset_header.Tables[0].Rows[0]["status"].ToString();


                //绑定Repeater中数据
                DataSet dataset_line = new DataSet();
                dataset_line = invoiceDC.getReturnLineBySome(select_text.Value, "", "", Convert.ToDateTime("2016-07-01 00:00:00"), DateTime.Now);

                printMaterialReturningRepeater.DataSource = dataset_line;
                printMaterialReturningRepeater.DataBind();
            }
            catch (Exception e)
            {
