...
for (var i = 0; i < btn_delete.length; i++) {
    ...
    btn_delete[i].onclick = function () {
        ...
        document.getElementById("subinventory_key_Delete").value = td[0].innerHTML;
...
for (var i = 0; i < btn_update.length; i++) {
    ...
    btn_update[i].onclick = function () {
        ...
        document.getElementById("subinventory_key_Update").value = td[0].innerHTML;
        document.getElementById("subinventory_name_Update").value = td[1].innerHTML;
...
$(document).ready(function () {
    $.ajax({
        ...
        async: false,
        type: "POST",
        url: "getAllSubinventory.ashx",
        dataType: "json",
        data: {
            warehouse: $("#subinventory_name_Query").val(),
        },
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                $("#subinventory_name_Query").append("<option>" + data[i].Name + "</option>");
            }
        }
    });
});
...
//        dataType: "json",
//        data: {
//            warehouse: $("#subinventory_name_Update").val(),
//        },
//        success: function (data) {
//            for (var i = 0; i < data.length; i++) {
//                $("#subinventory_name_Update").append("<option>" + data[i].Name + "</option>");
//            }
//        }
//    });
//});