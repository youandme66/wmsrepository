...
public class BaseActivity_clear extends Activity implements IBarcodeScan {

	private static final String TAG = "BaseActivity_clear";
		...
		runOnUiThread(new Runnable() {
			public void run() {
				...
				if (mCommReception != null) {
					...
					s = s.replace("�", "");// 替換亂碼
					//mCommReception.append(s);
					AppData.scandata=s;//賦值給變量,不賦值爲編輯框					
					StopScan();
				}
			}
		});
	...
	protected void handleOutmemoryError() {
		runOnUiThread(new Runnable() {
			// @Override
			public void run() {
				Toast.makeText(BaseActivity_clear.this, "内存空间不足！", Toast.LENGTH_SHORT)
						.show();
				// finish();
			}
		});
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				if (network_err_count < 3) {
					Toast.makeText(BaseActivity_clear.this, "网速好像不怎么给力啊！",
							Toast.LENGTH_SHORT).show();
				} else if (network_err_count < 5) {
					Toast.makeText(BaseActivity_clear.this, "网速真的不给力！",
							Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(BaseActivity_clear.this, "唉，今天的网络怎么这么差劲！",
							Toast.LENGTH_SHORT).show();
				}
				// finish();
			}
	...
	protected void handleMalformError() {
		runOnUiThread(new Runnable() {
			// @Override
			public void run() {
				Toast.makeText(BaseActivity_clear.this, "数据格式错误！", Toast.LENGTH_SHORT)
						.show();
				// finish();
			}
		});
	...
	protected void handleFatalError() {
		runOnUiThread(new Runnable() {
			// @Override
			public void run() {
				Toast.makeText(BaseActivity_clear.this, "发生了一点意外，程序终止！",
						Toast.LENGTH_SHORT).show();
				finish();
			}
		});
