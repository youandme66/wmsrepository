...
for (var i = 0; i < btn_update.length; i++) {
    ...
    btn_update[i].onclick = function () {
        ...
        document.getElementById("department_id_update").value = td[0].innerHTML;
        document.getElementById("flex_value_update").value = td[1].innerHTML;
        document.getElementById("description_update").value = td[2].innerHTML;
        document.getElementById("description_update_old").value = td[2].innerHTML;
        document.getElementById("enabled_update_id").value = td[3].innerHTML;
    }
}
...
for (var i = 0; i < btn_delete.length; i++) {
    ...
    btn_delete[i].onclick = function () {
        ...
        var tr = table.getElementsByTagName("tr")[this.index + 1];
        var td = tr.getElementsByTagName("td");
        document.getElementById("department_id_delete").value = td[0].innerHTML;
        //document.getElementById("flex_value_delete").innerHTML = td[1].innerHTML;
        document.getElementById("department_name_delete").innerHTML = td[2].innerHTML;
    }
}
...
$(document).ready(function () {
    $.ajax({
        async: false,
        type: "POST",
        url: "getDepartment.ashx",
        dataType: "json",
        data: {
            warehouse: $("#flex_value_select").val(),
        },
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                $("#flex_value_select").append("<option>" + data[i].Name + "</option>");
            }
        }
    });
});
...
$(document).ready(function () {
    $.ajax({
        async: false,
        type: "POST",
        url: "getDepartmentDescription.ashx",
        dataType: "json",
        data: {
            warehouse: $("#flex_value_update").val(),
        },
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                $("#flex_value_update").append("<option>" + data[i].Name + "</option>");
            }
        }
    });
});
...
$(document).ready(function () {
    $.ajax({
        ...
        dataType: "json",
        data: {
            warehouse: $("#description_select").val(),
        },
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                $("#description_select").append("<option>" + data[i].Name + "</option>");
            }
        }
    });
});
